...
#Attempting to use previous networks results to bootstrap next training
...
for seed in xrange(naveragingtrials):
	...
	c_individual_cross_entropy = -tf.reduce_sum(y_ * tf.log(c_y_conv),reduction_indices=[1])
	...
	ac_individual_cross_entropy = -tf.reduce_sum(y_ * tf.log(ac_y_conv),reduction_indices=[1])
	...
	if training_round == 1:
	    ...
	    c1_scores.append(best_c_score)
	    ac1_scores.append(best_ac_score)
	    print("standard test accuracy %g"%s_scores[-1])
	    print("curriculum 1 test accuracy %g"%c1_scores[-1])
	    ...
	    print("curriculum 2 test accuracy %g"%c2_scores[-1])
	    print("active curriculum 2 test accuracy %g"%ac2_scores[-1])
...
numpy.savetxt('c1_scores_ttss_%i_cs_%i_batch_%i_actl_%i_acaat_%f.csv'%(total_training_set_size,chunk_size,batch_size,ac_acc_track_length,ac_avg_acc_threshold),c1_scores,delimiter=',')
numpy.savetxt('ac1_scores_ttss_%i_cs_%i_batch_%i_actl_%i_acaat_%f.csv'%(total_training_set_size,chunk_size,batch_size,ac_acc_track_length,ac_avg_acc_threshold),ac1_scores,delimiter=',')
numpy.savetxt('c2_scores_ttss_%i_cs_%i_batch_%i_actl_%i_acaat_%f.csv'%(total_training_set_size,chunk_size,batch_size,ac_acc_track_length,ac_avg_acc_threshold),c2_scores,delimiter=',')
numpy.savetxt('ac2_scores_ttss_%i_cs_%i_batch_%i_actl_%i_acaat_%f.csv'%(total_training_set_size,chunk_size,batch_size,ac_acc_track_length,ac_avg_acc_threshold),ac2_scores,delimiter=',')
